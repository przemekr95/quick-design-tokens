// Ten plik konwertuje tokeny projektowe do zmiennych SCSS
import fs from 'fs';
...
// Ścieżka do zbudowanych plików tokenów
const builtTokensPath = path.resolve(rootDir, 'dist/esm/tokens/index.js');

async function main() {
  // Dynamiczny import tokenów ze zbudowanych plików
  const tokenModule = await import(builtTokensPath);
...
// Uruchom główną funkcję i obsłuż błędy
main().catch((error) => {
